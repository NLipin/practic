...
function Circle(id, positionX, positionY, radius, fill, stroke, stroke_width) {
    ...
    this.draw = function(){
        ...
        `<circle id="${id}" cx="${this.positionX}" cy="${this.positionY}" r="${this.radius}" fill="${this.fill}" stroke="${this.stroke}" stroke-width="${this.stroke_width}" />`;
    }
}
function randColor() {
...
function surfaceBilding() {
    let outerCircleArray = [];
    let innerCircleArray = [];
    for (let j = i; j < quantRow; j++){
        for (let k=1; k< quantColumn; k++) {
         i++;
         outerCircleArray[i] = new Circle(i, k * nodesDistance, j * nodesDistance, 10, "black", randColor(), 2);
         outerCircleArray[i].draw();
 
         innerCircleArray[i] = new Circle("k" + i, k * nodesDistance, j * nodesDistance, 1, "yellow",randColor(), 1);
